...
// Vérifie si un ID d'événement est spécifié dans l'URL
if (isset($_GET['id_evenement'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        // Redirige si l'événement n'existe pas
        header("Location: Evenements.php");
        exit();
    }
} else {
    // Redirige si l'ID d'événement n'est pas spécifié
    header("Location: Evenements.php");
    exit();
}

// Gérer la mise à jour de l'événement
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'enregistrer') {
        $nouveauTitre = $_POST['titre'];
        $nouveauxDetails = $_POST['details_evenement'];

        // Exécuter la requête pour mettre à jour l'événement
        $sql = "UPDATE evenement SET titre = '$nouveauTitre', details_evenement = '$nouveauxDetails' WHERE id_evenement = $id_evenement";
        $result = $mysqli->query($sql);
        ...
        // Rediriger vers la liste des événements si l'annulation est sélectionnée
        header("Location: Evenements.php");
        exit();
    }
}
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                <div class="d-sm-flex justify-content-between align-items-center mb-4" style="margin-bottom: 25px;margin-top: 30px;">
                    <h3 class="text-dark mb-0" style="margin-left: 30px;">Modifier évènement</h3>
                </div>
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3">
                            <label for="details_evenement" class="form-label">Détails de l'événement</label>
                            <textarea class="form-control" id="details_evenement" name="details_evenement" rows="5"><?php echo $evenement['details_evenement']; ?></textarea>
                        </div>
